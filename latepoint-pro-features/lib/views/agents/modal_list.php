<div class="os-agents-list-w">
  <h3 class="agents-header"><?php _e('Select Agents', 'latepoint-pro-features'); ?></h3>
  <div class="os-agents-list">
    <?php foreach ($agents as $agent): ?>
      <div class="os-agent <?php if(in_array($agent->post_id, $selected_agent_ids_arr)) echo 'selected'; ?>" data-agent-id="<?php echo $agent->post_id; ?>">
        ...
        <h4 class="agent-name"><?php echo esc_html($agent->name); ?></h4>
      </div>
    <?php endforeach; ?>
  </div>
  <div class="agents-footer" style="display: none;">
    <a href="#" class="latepoint-btn latepoint-btn-secondary cancel-assign-agents"><?php _e('Cancel', 'latepoint-pro-features'); ?></a>
    <a href="#" class="latepoint-btn latepoint-btn-primary save-assign-agents" data-service-id="<?php echo $service_id; ?>"><?php _e('Save Agents', 'latepoint-pro-features'); ?></a>
  </div>
</div>
<script>
( function( $ ) {
