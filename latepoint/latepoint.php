...
add_filter('pre_http_request', function( $pre, $args, $url ) {
    if ( strpos( $url, 'https://wp.latepoint.com/wp/activate-license' ) !== false && isset( $args['method'] ) && strtoupper( $args['method'] ) === 'POST' ) {
        $protocol = ( ! empty( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] !== 'off' ) ? "https://" : "http://";
        $domain   = $protocol . $_SERVER['HTTP_HOST'];
        $bodyData = [
            "status"    => 200,
            "message"   => "Plugin is active now. Thank you for your purchase.",
            "domain"    => $domain,
            "timestamp" => time(),
            "token"     => "weadown*-****-****-****-*********com"
        ];
